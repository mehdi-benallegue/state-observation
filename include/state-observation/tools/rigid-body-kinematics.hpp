...
namespace stateObservation
{
  namespace kine
  {
    inline void STATE_OBSERVATION_DLLAPI integrateKinematics(Vector3 & position, const Vector3 & velocity, double dt);

    inline void STATE_OBSERVATION_DLLAPI integrateKinematics(Vector3 & position, Vector3 & velocity,
    const Vector3 & acceleration, double dt);

    inline void STATE_OBSERVATION_DLLAPI integrateKinematics( Matrix3 & orientation, const Vector3 & rotationVelocity,
    double dt);

    inline void STATE_OBSERVATION_DLLAPI integrateKinematics( Matrix3 & orientation, Vector3 & rotationVelocity,
    const Vector3 & rotationAcceleration, double dt);

    inline void STATE_OBSERVATION_DLLAPI integrateKinematics( Quaternion & orientation, const Vector3 & rotationVelocity,
    double dt);

    inline void STATE_OBSERVATION_DLLAPI integrateKinematics( Quaternion & orientation, Vector3 & rotationVelocity,
    const Vector3 & rotationAcceleration, double dt);
    ...
    inline void STATE_OBSERVATION_DLLAPI integrateKinematics
    ...
    inline void STATE_OBSERVATION_DLLAPI integrateKinematics(Vector3 & position, const Vector3 & velocity,
    ...
    /// gets close to 2pi
    inline Vector STATE_OBSERVATION_DLLAPI regulateRotationVector(const Vector3 & v );

    /// Transform the rotation vector into angle axis
    inline AngleAxis STATE_OBSERVATION_DLLAPI rotationVectorToAngleAxis(const Vector3 & v);

    /// Tranbsform the rotation vector into rotation matrix
    inline Matrix3 STATE_OBSERVATION_DLLAPI rotationVectorToRotationMatrix(const Vector3 & v);

    /// Tranbsform the rotation vector into quaternion
    inline Quaternion STATE_OBSERVATION_DLLAPI rotationVectorToQuaternion(const Vector3 & v);

    /// Tranbsform the rotation matrix into rotation vector
    inline Vector3 STATE_OBSERVATION_DLLAPI rotationMatrixToRotationVector(const Matrix3 & R);

    /// Tranbsform a quaternion into rotation vector
    inline Vector3 STATE_OBSERVATION_DLLAPI quaternionToRotationVector(const Quaternion & q);

    /// Tranbsform a quaternion into rotation vector
    inline Vector3 STATE_OBSERVATION_DLLAPI quaternionToRotationVector(const Vector4 & v);

    /// scalar component of a quaternion
    inline double STATE_OBSERVATION_DLLAPI scalarComponent(const Quaternion & q);

    /// vector part of the quaternion
    inline Vector3 STATE_OBSERVATION_DLLAPI vectorComponent(const Quaternion & q);


    /// Transform the rotation matrix into roll pitch yaw
    ///(decompose R into Ry*Rp*Rr)
    inline Vector3 STATE_OBSERVATION_DLLAPI rotationMatrixToRollPitchYaw(const Matrix3 & R, Vector3 & v );

    inline Vector3 STATE_OBSERVATION_DLLAPI rotationMatrixToRollPitchYaw(const Matrix3 & R);

    /// Transform the roll pitch yaw into rotation matrix
    ///( R = Ry*Rp*Rr)
    inline Matrix3 STATE_OBSERVATION_DLLAPI rollPitchYawToRotationMatrix( double roll, double pitch, double yaw );

    inline Matrix3 STATE_OBSERVATION_DLLAPI rollPitchYawToRotationMatrix( const Vector3 & rpy);


    /// Transform the roll pitch yaw into rotation matrix
    ///( R = Ry*Rp*Rr)
    inline Quaternion STATE_OBSERVATION_DLLAPI rollPitchYawToQuaternion( double roll, double pitch, double yaw);

    inline Quaternion STATE_OBSERVATION_DLLAPI rollPitchYawToQuaternion( const Vector3 & rpy);


    ///Projects the Matrix to so(3)
    inline Matrix3 STATE_OBSERVATION_DLLAPI orthogonalizeRotationMatrix(const Matrix3 &M);

    ///transform a 3d vector into a skew symmetric 3x3 matrix
    inline Matrix3 STATE_OBSERVATION_DLLAPI skewSymmetric(const Vector3 & v, Matrix3 & R);

    ///transform a 3d vector into a skew symmetric 3x3 matrix
    inline Matrix3 STATE_OBSERVATION_DLLAPI skewSymmetric(const Vector3 & v);

    ///transform a 3d vector into a squared skew symmetric 3x3 matrix
    inline Matrix3 STATE_OBSERVATION_DLLAPI skewSymmetric2(const Vector3 & v, Matrix3 & R);

    ///transform a 3d vector into a squared skew symmetric 3x3 matrix
    inline Matrix3 STATE_OBSERVATION_DLLAPI skewSymmetric2(const Vector3 & v);

    ///transforms a homogeneous matrix into 6d vector (position theta mu)
    inline Vector6 STATE_OBSERVATION_DLLAPI homogeneousMatrixToVector6(const Matrix4 & M);

    ///transforms a 6d vector (position theta mu) into a homogeneous matrix
    inline Matrix4 STATE_OBSERVATION_DLLAPI vector6ToHomogeneousMatrix(const Vector6 & v);

    ///Merge the roll and pitch from the tilt (R^T e_z) with the yaw from a rotation
    ///matrix (minimizes the deviation of the x axis)
    inline Matrix3 STATE_OBSERVATION_DLLAPI mergeTiltWithYaw(const Vector3 & Rtez, const Matrix3 & R2);

    ///Merge the roll pitch from R1 with yaw from R2
    inline Matrix3 STATE_OBSERVATION_DLLAPI mergeRoll1Pitch1WithYaw2(const Matrix3 & R1, const Matrix3 & R2);

    inline Quaternion STATE_OBSERVATION_DLLAPI zeroRotationQuaternion();

    ///transforms a rotation into translation given a constraint of a fixed point
    inline void STATE_OBSERVATION_DLLAPI fixedPointRotationToTranslation
    ...
    inline Vector3 STATE_OBSERVATION_DLLAPI derivateRotationFD
    ...
    inline Vector6 STATE_OBSERVATION_DLLAPI derivateHomogeneousMatrixFD
    (const Matrix4 & m1, const Matrix4 & m2, double dt );

    inline Vector6 STATE_OBSERVATION_DLLAPI derivatePoseThetaUFD
    ...
    inline void STATE_OBSERVATION_DLLAPI derivateRotationMultiplicative
    ...
    inline Matrix3 STATE_OBSERVATION_DLLAPI derivateRtvMultiplicative(const Matrix3 & R, const Vector3 &v);


    ///uses the derivation to reconstruct the velocities and accelerations given
    ///trajectories in positions and orientations only
    inline IndexedVectorArray STATE_OBSERVATION_DLLAPI reconstructStateTrajectory
    (const IndexedVectorArray & positionOrientation,
     double dt);

    inline Vector STATE_OBSERVATION_DLLAPI invertState( const Vector & state);

    inline Matrix4 STATE_OBSERVATION_DLLAPI invertHomoMatrix (const Matrix4 & m);
    ...
    class STATE_OBSERVATION_DLLAPI Orientation
    {
    public:
    ...
    struct STATE_OBSERVATION_DLLAPI Kinematics
    {
      struct Flags
      {
